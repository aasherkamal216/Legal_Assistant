...
def trigger_content_generation():
    ...
    if not st.session_state.get("messages") or len(st.session_state.messages) < 2:
        st.toast("Conversation is too short for analysis.", icon="ℹ️")
        return
...
def user_type_selection():
    ...
    with col1:
        st.markdown('<div style="text-align: center; padding: 20px; border: 2px solid #1f77b4; border-radius: 10px; margin: 10px 0;"><h3>🎓 Legal Professional</h3><p style="font-size: 14px; color: #666;">For lawyers and legal practitioners seeking detailed legal analysis, case law references, and professional-level insights.</p></div>', unsafe_allow_html=True)
