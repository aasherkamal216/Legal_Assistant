...
from .faq_generator import FAQ_PROMPT
...
class FAQGenerator:
    ...
    def generate_faqs_from_conversation(self, conversation_history: List[Dict[str, str]]) -> List[Dict[str, str]]:
        ...
        result = faq_generator.invoke(
            [
                SystemMessage(
                    content=FAQ_PROMPT.format(conversation=formatted_conversation)
                ),
                HumanMessage(content=formatted_conversation),
            ]
        )

        # Convert to dict format and store in database
        faq_dicts = [
            {
